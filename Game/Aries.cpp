...
Aries::Aries(EntityType type, EntityFaction faction, Map* owner, const Vec2& startPos, float orientationDegrees)
...
{
	float ariesDriveSpeed = g_gameConfigBlackboard.GetValue("ariesDriveSpeed", 0.5f);
	float ariesTurnRate = g_gameConfigBlackboard.GetValue("ariesTurnRate", 180.f);
	float ariesWanderRandomizePeriod = g_gameConfigBlackboard.GetValue("ariesWanderRandomizePeriod", 180.f);
	if (m_health <= 0)
	{
	...
	if (!CheckPlayerInSight())
	{
		...
		{
			if (m_randomTime >= ariesWanderRandomizePeriod)
			{
		...
		else
		{
			...
			else if (!m_knowPlayerBefore && m_randomTime >= ariesWanderRandomizePeriod)
			{
				FindPathToRandomTarget();
				m_randomTime = 0.f;
			}
	...
	m_orientationDegreesTank = GetTurnedTowardDegrees(m_orientationDegreesTank, m_Degrees, ariesTurnRate * deltaSeconds);
	Vec2 forwardDirection = GetForwardNormal();
	m_velocity += forwardDirection * ariesDriveSpeed * deltaSeconds;
	m_position += m_velocity;
}

void Aries::Render() const
{
	Vertex_PCU tempTankWorldVerts[NUM_ARIES_VERTS];
	for (int vertIndex = 0; vertIndex < NUM_ARIES_VERTS; ++vertIndex)
	{
		tempTankWorldVerts[vertIndex] = m_localVertsTank[vertIndex];
	}

	TransfromVertexArrayXY3D(NUM_ARIES_VERTS, tempTankWorldVerts, 1.f, m_orientationDegreesTank, m_position);
	g_theRenderer->BindTexture(m_tankTexture);
	g_theRenderer->DrawVertexArray(NUM_ARIES_VERTS, tempTankWorldVerts);
...
void Aries::ReactToBulletHit(Bullet& bullet)
{
	...
	if (GetAngleDegreesBetweenVectors2D(dispToBullet, fwdNormal) < 45.f)
	{
		g_theAudio->StartSound(m_theGame->m_bulletRicochetAudio);
		PushDiscOutOfFixedDisc2D(bullet.m_position, bullet.m_physicsRadius, m_position, m_physicsRadius);
		Vec2 bounceSurfaceNormal = dispToBullet.GetNormalized();
		if (bullet.GetEntityType() == ENTITY_TYPE_GOOD_FIRES || bullet.GetEntityType() == ENTITY_TYPE_EVIL_FIRES)
		{
			bullet.Die();
		}
		else if (bullet.GetEntityType() == ENTITY_TYPE_EVIL_BULLET || bullet.GetEntityType() == ENTITY_TYPE_GOOD_BULLET)
		{
			bullet.BounceOff(bounceSurfaceNormal);
		}
	}
	else
	{
...
void Aries::InitializeLocalVertsForTank()
{
...
void Aries::FindPathToTarget(Vec2 startPos)
{
...
void Aries::FindPathToRandomTarget()
{
