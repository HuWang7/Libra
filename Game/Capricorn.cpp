...
Capricorn::Capricorn(EntityType type, EntityFaction faction, Map* owner, const Vec2& startPos, float orientationDegrees)
...
{
	float leoDriveSpeed = g_gameConfigBlackboard.GetValue("capricornDriveSpeed", 0.5f);
	float leoTurnRate = g_gameConfigBlackboard.GetValue("capricornTurnRate", 90.f);
	float leoWanderRandomizePeriod = g_gameConfigBlackboard.GetValue("capricornWanderRandomizePeriod", 1.0f);
	float leoShootCooldownSeconds = g_gameConfigBlackboard.GetValue("capricornShootCooldownSeconds", 1.3f);
	if (m_health <= 0)
	{
	...
	else
	{
		...
		if (angleBetweenPlrAndEnm < 5.f)
		{
			...
			if (m_fireTime >= leoShootCooldownSeconds)
			{
				...
				m_theGame->m_currentMap->SpawnNewEntity(ENTITY_TYPE_EVIL_MISSILE, gunpoint, m_orientationDegreesTank);
...
void Capricorn::InitializeLocalVertsForTank()
{
...
bool Capricorn::CheckPlayerInSight()
{
...
void Capricorn::FindPathToTarget(Vec2 startPos)
{
	m_map->EnemyPopulateDistanceField(m_heatMap, IntVec2((int)startPos.x, (int)startPos.y), 999.f, false);
...
void Capricorn::FindPathToRandomTarget()
{
